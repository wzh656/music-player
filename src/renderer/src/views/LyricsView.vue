...
const currentLyrics = inject("currentLyrics") as Ref<string | null>; //当前播放歌词
...
watch(currentMusic, async () => {
  ...
  console.log("[LyricsView] currentMusic change", currentMusic.value);

  //获取歌词
  let text = "";
  if (currentMusic.value.startsWith("http")) {
    text = currentLyrics.value ?? "";
  } else {
    text = await window.electron.getLyrics(currentMusic.value);
  }
